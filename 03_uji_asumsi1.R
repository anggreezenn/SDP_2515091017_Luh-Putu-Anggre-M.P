...
if(!kolom_uji %in% names(data_startup_saas)) {
...
# Membuat Q-Q Plot
qq_plot <- ggplot(data_startup_saas, aes_string(sample = kolom_uji)) +
  stat_qq() +
...
if(nrow(data_startup_saas) < 5000) {
  shapiro_test_result <- shapiro.test(data_startup_saas[[kolom_uji]])
