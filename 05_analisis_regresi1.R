...
if(!var_dependen %in% names(data_startup_saas) || !var_independen %in% names(data_startup_saas)) {
...
model_regresi <- lm(as.formula(paste(var_dependen, "~", var_independen)), data = data_startup_saas)
...
plot_regresi <- ggplot(data_startup_saas, aes_string(x = var_independen, y = var_dependen)) +
