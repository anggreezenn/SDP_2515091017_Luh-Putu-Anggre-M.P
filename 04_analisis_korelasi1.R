...
if(!var_x %in% names(data_startup_saas) || !var_y %in% names(data_startup_saas)) {
...
scatter_plot <- ggplot(data_startup_saas, aes_string(x = var_x, y = var_y)) +
...
correlation_test <- cor.test(data_startup_saas[[var_x]], data_startup_saas[[var_y]], method = "pearson")
